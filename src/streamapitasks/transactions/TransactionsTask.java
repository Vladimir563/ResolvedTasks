package streamapitasks.transactions;
...
public class TransactionsTask
{
    public static void main(String[] args)
    {
        ...
        Map<String, Long> accIntMap = transactionStream
            .collect(Collectors.groupingBy(item -> item.getAccount().getNumber(), Collectors.summingLong((sum -> sum.getSum()))));
        accIntMap.entrySet().forEach(System.out::println);
        System.out.println();

        // TODO: 2. найти сумму транзакций по каждому аккаунту

        long transactionsSum = accIntMap.entrySet().stream()
                .collect(Collectors.summingLong(acc -> acc.getValue()));
        System.out.println("Сумма транзакций по всем аккаунтам: " + transactionsSum);
    }
}